/* Author: Timothy Kelley
   ...
   and stores the information in the Clients class. Member functions such
   as deleting a client, adding a new one, and printing bills can
   be used through the menu system. Execution ends with overwritting
   vetDB.dat with stored data from the clients object in the same 
   format.
*/
...
//class for the client array and member functions using it  
class Clients {
    private:
        int nextAcctNum;
        int count;
        ...
        int findClient(int acctNum);
    public:
        Clients();
        void readClients();
        void printBill(int acctNum);
        void printPet(int acctNum);
        void addClient();
        void deleteClient(int acctNum);
        void printAll();
        void saveDB();
...
int main () {     
    Clients clients; //Clients class with client array
    int choice;      //user input for menu
    int acct;        //Input acct. number for search
    ...
    while(!inExit) { 
        ...
        switch(choice) { 
            ...
            case 2: cout << "\n\nWhat is the account number ";
                    cin >> acct;
                    clients.printPet(acct);
                    break;
                    
            //Choice 3: adds client to client array by calling addClient 
            case 3: clients.addClient();
                    break;
...
Client Clients::readOne(istream & inFile) {
...
int Clients::findClient(int acctNum) {
...
void Clients::deleteClient(int acctNum) {
    //find index of matching account number
    int i = findClient(acctNum);  
...
void Clients::printBill(int acctNum) {
    int i = findClient(acctNum);  //integer for index of searching account number
...
void Clients::printPet(int acctNum) {
    int i = findClient(acctNum);  //the index of matching account number
